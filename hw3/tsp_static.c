...
int recurseSolve(int curInd, int curWeight, int* path, int* used, int* bestPath,int citiesNum) {
...
int solve(int prefix[], int len, int initialWeight, int* bestPath,int citiesNum) {
	int used[citiesNum];
	...
	for(int i = 0; i < len; ++i)
		used[prefix[i]] = 1;
	return recurseSolve(len - 1, initialWeight, path, used, bestPath, citiesNum);
}
...
int tsp_main(int citiesNum, int xCoord[], int yCoord[], int shortestPath[]) {
  ...
  if(citiesNum < 6) {
    ...
    int minWeight = solve(prefix, 1, 0, bestPath,citiesNum);
